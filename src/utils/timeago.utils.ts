...
class TimeAgo {
  ...
  chatMessageTransform(value: Date): string {
    const date = typeof value === "string" ? new Date(value) : value;
  ...
  dayMonthYear(value: Date): string {
    const date = typeof value === "string" ? new Date(value) : value;
    return format(date, "d MMMM yyyy");
  }

  timeFormat(value: Date | string): string {
    const date = typeof value === "string" ? new Date(value) : value;
    return format(date, "HH:mm a");
  }

  timeDifference(current: Date, date: Date): string {
  ...
  secondsAgo(elapsed: number): string {
  ...
  minutesAgo(elapsed: number, msPerMinute: number): string {
  ...
  hoursAgo(elapsed: number, msPerHour: number): string {
  ...
  monthsAgo(date: Date, elapsed: number, msPerDay: number): string {
